             ...
             <?php
                    if(isset($_GET['pid'])){
                      ...
                      if(mysqli_num_rows($Fur_pr_run) > 0){
                         $Fur_pr_row = mysqli_fetch_array($Fur_pr_run);
                          ...
                          $db_image_another = $Fur_pr_row['anotherimg'];
                          ...
                          if(isset($_POST['update'])){ 
                            ...
                            $anoimage      = $_FILES['anotherupload']['name'];
                            $anotmp_image  = $_FILES['anotherupload']['tmp_name'];
                                
                            if(!empty($title) || !empty($size) || !empty($price) || !empty($status) || !empty($category) || !empty($detail) || !empty($image) || !empty($anoimage)){
                              if($image==''){
                                $image=$db_image;
                              }
                              if($anoimage==''){
                                $anoimage=$db_image_another;
                              }
                              $query = "UPDATE furniture_product SET title='$title', category=$category, size='$size', price =$price, detail='$detail', image='$image',anotherimg='$anoimage', status='$status' Where pid=$Fur_pro_id";                            
                                if(mysqli_query($con,$query)){
                                    $path = "img/".$image;
                                    if(move_uploaded_file($tmp_image,$path) == true){
                                        copy($path,"../".$path);                                                                    
                                    }
                                    $anopath = "img/".$anoimage;
                                    if(move_uploaded_file($anotmp_image,$anopath) == true){
                                      copy($anopath,"../".$anopath);
              ...
              <div class="row mt-3">
                ...
                <div class="col-md-6">      
                  ...
                  <label for="size">Thumbanil Image</label>
              ...
              <div class="row mt-3">
                ...
                <div class="col-md-6">      
                  ...
                  <label for="size"> Image</label>
                     <input type="file" name="anotherupload" class="form-control-file border" >
                </div>
                <div class="col-md-6">
                  <img src="img/<?php echo $db_image_another ;?>" min-width="100%"  height="200px">
                </div>
